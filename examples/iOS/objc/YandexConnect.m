...
@implementation YandexDiskConnect
...
- (void)viewDidLoad {
		...
		char *error = NULL;
		char *urlstr = 
				c_yandex_oauth_code_url(CLIENTID);
		if (urlstr){
...
int save_token(void *data, const char *token, time_t expires, const char *refresh, const char *error){
	YandexDiskConnect *self = data;
	if (error){
		NSLog(@"%s", error);
	}
	if (token){
